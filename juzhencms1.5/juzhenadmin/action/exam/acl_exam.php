...
class acl_exam extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->exam,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
	...
	function listsData(){
		...
		$where=" from {$dao->tables->exam} where 1=1 ";
		if($_REQUEST["publish"]!='')
		...
		$list_sql="select auto_id,content_title,publish,position,create_user,create_time "
		.$where.$orderby;
		...
		foreach($list as $key=>$vl){
			...
			//获取答题数量
			$tmp=$dao->get_datalist("select count(auto_id) as count_num from {$dao->tables->exam_record} where pid='{$vl["auto_id"]}'");
			$list[$key]["record_num"]=$tmp[0]["count_num"];
		}
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->exam,$dataArray);			
		}else{
			$dao->update($dao->tables->exam,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function delete(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			if(is_array($idArr) && count($idArr)>0){
				...
				$idStr=implode(',',$idArr);
				$dao->delete($dao->tables->exam,"where auto_id in({$idStr})");
			}
		}		
		//$this->redirect("?acl={$_REQUEST["acl"]}&method=lists");
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->exam,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
}
