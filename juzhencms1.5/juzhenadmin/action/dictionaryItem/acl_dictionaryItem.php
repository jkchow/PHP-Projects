...
class acl_dictionaryItem extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->dictionary_item,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
			$recordData["publish"]='1';
			$recordData["dictionary_id"]=$_REQUEST["dictionary_id"];
	...
	function listsData(){
		...
		$where=" from {$dao->tables->dictionary_item} where 1=1 ";
		
		if($_REQUEST["dictionary_id"]!="")
			$where.=" and dictionary_id='{$_REQUEST["dictionary_id"]}'";
		...
		$list_sql="select auto_id,dictionary_id,content_text,content_value,publish,position,create_user,create_time "
		.$where.$orderby;
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->dictionary_item,$dataArray);			
		}else{
			$dao->update($dao->tables->dictionary_item,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->dictionary_item,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
	...
	function delete(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			if(is_array($idArr) && count($idArr)>0){
				...
				$idStr=implode(',',$idArr);
				$dao->delete($dao->tables->dictionary_item,"where auto_id in({$idStr})");
			}
		}	
		//$backUrl=base64_decode($_REQUEST["backUrl"]);	
		//$this->redirect($backUrl);
