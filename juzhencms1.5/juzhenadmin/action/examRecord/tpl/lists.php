    ...
    <script type="text/javascript">
        ...
        $(function () {			
            grid = $("#maingrid").ligerGrid({
                ...
                { display: 'ip地址', name: 'content_ip', minWidth: 100,align:'center' }, 
                { display: '会员', name: 'member_id', minWidth: 100 },
				
                { display: '创建时间', name: 'create_time' },
				{ display: '操作', isSort: false, width: 180, render: function (rowdata, rowindex, value)
                {
                    var h = "";
					h += "<a href='javascript:view_item(" + rowdata.auto_id + ")'>答题记录</a> ";
					h += "<a href='javascript:edit_record(" + rowdata.auto_id + ")'>测试结果</a> ";
		...
		function view_item(recordID){
			...
			//跳转到广告列表
			var url="?acl=examRecordAnswer&method=lists&pid="+recordID;
			CmsTools.pageJump(url,true);
		}
		function goBack(){
			CmsTools.jumpBack();
