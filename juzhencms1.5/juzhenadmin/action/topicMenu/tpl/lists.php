...
<head>
    ...
    <script type="text/javascript">
		var grid;
		var dataUrl="?acl=topicMenu&method=treeGridJsonData";
        ...
        $(function () {			
            grid = $("#maingrid").ligerGrid({
				...
				{ display: '功能', name: 'content_module', minWidth: 70,type:"enumVars",enumVars:enumVars.topicMenuModuleVars },
				{ display: '发布状态', name: 'publish', minWidth: 60,type:"enumVars",enumVars:enumVars.publishVars },
                { display: '创建时间', name: 'create_time' },
				{ display: '操作', isSort: false, minWidth: 100, render: function (rowdata, rowindex, value)
					{
						var h = "";
						if(rowdata.menu_key=="0"){	
		...
		function add_record(){
			var url="?acl=topicMenu&method=form";
			CmsTools.pageJump(url);
		}
		function edit_record(recordID){
			if(recordID!=null){
				var url="?acl=topicMenu&method=form&auto_id="+recordID;
				CmsTools.pageJump(url);
			}else{
				...
				if (!rowdata)
					...
					var url="?acl=topicMenu&method=form&auto_id="+rowdata.auto_id;
					CmsTools.pageJump(url);
				}
			}
		}
		...
		function up()
        {
			...
			$.ajax({
			   type: "get",
			   url: "?acl=topicMenu&method=position_up&auto_id="+row.auto_id
			});
			collapseFlag=false;
            grid.up(row);
			collapseFlag=true;
        ...
        function down()
        {
			...
			$.ajax({
			   type: "get",
			   url: "?acl=topicMenu&method=position_down&auto_id="+row.auto_id
			});
			collapseFlag=false;
            grid.down(row);
			collapseFlag=true;
		...
		function addTestData()
        {
			...
			$.ajax({
			   type: "get",
			   url: "?acl=topicMenu&method=addTestData&auto_id="+row.auto_id,
