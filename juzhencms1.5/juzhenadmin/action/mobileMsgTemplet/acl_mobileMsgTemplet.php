...
class acl_mobileMsgTemplet extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->mobile_msg_templet,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
	...
	function listsData(){
		...
		$where=" from {$dao->tables->mobile_msg_templet} where 1=1 ";
		if($_REQUEST["publish"]!='')
		...
		$list_sql="select auto_id,content_name,content_event,publish,position,create_user,create_time "
		.$where.$orderby;
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->mobile_msg_templet,$dataArray);			
		}else{
			$dao->update($dao->tables->mobile_msg_templet,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function delete(){
		global $dao;
		if($_REQUEST["auto_id"]!="")
			$dao->delete($dao->tables->mobile_msg_templet,"where auto_id='{$_REQUEST["auto_id"]}'");	
		//$this->redirect("?acl={$_REQUEST["acl"]}&method=lists");
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->mobile_msg_templet,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
	...
	function sendMsg(){
		...
		if($_POST["member_mobile"]=="" || $_POST["templet_id"]==""){
			echo json_encode(array("result"=>0,"msg"=>"请选择会员及模板"));
		}else{
			$mobile_arr=explode(';',$_POST["member_mobile"]);
			$mobileTemplet=$dao->get_row_by_where($dao->tables->mobile_msg_templet,"where auto_id='{$_POST["templet_id"]}'");
			$mobileMsgService=new mobileMsgService();
			if(is_array($mobileTemplet) && is_array($mobile_arr))
			foreach($mobile_arr as $key=>$vl){
				$mobileMsgService->sendMsg($vl,$mobileTemplet["content_text"]);
			}
