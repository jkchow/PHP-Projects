...
class acl_memberAddress extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->member_address,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
	...
	function listsData(){
		...
		$where=" from {$dao->tables->member_address} where 1=1 ";
	...
	function save(){
		...
		if($dataArray["is_default"]=="1"){
			$tmpParam=array();
			$tmpParam["is_default"]="0";
			$dao->update($dao->tables->member_address,$tmpParam,"where member_id='{$dataArray["member_id"]}'");
		}
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->member_address,$dataArray);			
		}else{
			$dao->update($dao->tables->member_address,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function delete(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			if(is_array($idArr) && count($idArr)>0){
				...
				$idStr=implode(',',$idArr);
				$dao->delete($dao->tables->member_address,"where auto_id in({$idStr})");
			}
		}	
		//$backUrl=base64_decode($_REQUEST["backUrl"]);	
		//$this->redirect($backUrl);
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->member_address,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
