...
class acl_topicLinks extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->topic_links,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
	...
	function listsData(){
		...
		$where=" from {$dao->tables->topic_links} where 1=1 ";
		...
		if($_REQUEST["sortname"]!="" && $_REQUEST["sortorder"]!="" ){
			$orderby.=" order by {$_REQUEST["sortname"]} {$_REQUEST["sortorder"]} ,auto_id asc";
		}else{
		...
		$list_sql="select auto_id,content_name,content_link,publish,position,create_user,create_time "
		.$where.$orderby;
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->topic_links,$dataArray);			
		}else{
			$dao->update($dao->tables->topic_links,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->topic_links,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
	...
	function delete(){
		global $dao;
		if($_REQUEST["auto_id"]!="")
			$dao->delete($dao->tables->topic_links,"where auto_id='{$_REQUEST["auto_id"]}'");	
		//$this->redirect("?acl={$_REQUEST["acl"]}&method=lists");
