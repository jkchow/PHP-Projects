...
class acl_role extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->role,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
	...
	function listsData(){
		...
		$where=" from {$dao->tables->role} where 1=1 ";
		$where.=" and (content_value!='root' or content_value is null) ";//列表不显示超级管理员
		if($_REQUEST["publish"]!='')
		...
		$list_sql="select auto_id,content_name,publish,create_user,create_time "
		.$where.$orderby;
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->role,$dataArray);
		}else{
			$dao->update($dao->tables->role,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
		//$this->redirect("?acl=role&method=lists");
	...
	function delete(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			if(is_array($idArr) && count($idArr)>0){
				...
				$idStr=implode(',',$idArr);
				$dao->delete($dao->tables->role,"where auto_id in({$idStr})");
			}
		}
		//$this->redirect("?acl=role&method=lists");
	...
	function permissionJsonData(){
		$passport=new Passport();
		$rolePermissionsData=$passport->getRolePermissionsFormData($_REQUEST["auto_id"]);
		
		/*echo "<pre>";
		print_r($rolePermissionsData);
		exit;*/
		
		echo json_encode($rolePermissionsData);
	}
}
