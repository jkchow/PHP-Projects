...
class acl_topicAdSpace extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->topic_ad_space,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
	...
	function listsData(){
		...
		$where=" from {$dao->tables->topic_ad_space} where 1=1 ";
		...
		$list_sql="select auto_id,content_name,content_width,content_height,content_type,position,create_user,create_time "
		.$where.$orderby;
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->topic_ad_space,$dataArray);			
		}else{
			$dao->update($dao->tables->topic_ad_space,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
			
		//$this->redirect("?acl=topicAdSpace&method=lists");	
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->topic_ad_space,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
	...
	function delete(){
		global $dao;
		if($_REQUEST["auto_id"]!="")
			$dao->delete($dao->tables->topic_ad_space,"where auto_id='{$_REQUEST["auto_id"]}'");	
		//$this->redirect("?acl=topicAdSpace&method=lists");
