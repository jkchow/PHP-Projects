...
class acl_ad extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->ad,"where auto_id='{$_REQUEST["auto_id"]}'");
			if($recordData["menu_id"]=="0")
				$recordData["menu_id"]=NULL;
		}else{
			$recordData=array();
	...
	function listsData(){
		...
		$where=" from {$dao->tables->ad} where 1=1 ";
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->ad,$dataArray);			
		}else{
			$dao->update($dao->tables->ad,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->ad,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
	...
	function delete(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			if(is_array($idArr) && count($idArr)>0){
				...
				$idStr=implode(',',$idArr);
				$dao->delete($dao->tables->ad,"where auto_id in({$idStr})");
			}
		}		
		//$backUrl=base64_decode($_REQUEST["backUrl"]);	
		//$this->redirect($backUrl);	
