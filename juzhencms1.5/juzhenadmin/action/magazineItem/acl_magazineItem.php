...
class acl_magazineItem extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->magazine_item,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
			$recordData["publish"]='1';
			$recordData["magazine_id"]=$_REQUEST["magazine_id"];
	...
	function listsData(){
		...
		$where=" from {$dao->tables->magazine_item} where 1=1 ";
		
		if($_REQUEST["magazine_id"]!="")
			$where.=" and magazine_id='{$_REQUEST["magazine_id"]}'";
		...
		$list_sql="select auto_id,magazine_id,content_name,publish,position,create_user,create_time "
		.$where.$orderby;
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->magazine_item,$dataArray);			
		}else{
			$dao->update($dao->tables->magazine_item,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->magazine_item,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
	...
	function delete(){
		global $dao;
		if($_REQUEST["auto_id"]!="")
			$dao->delete($dao->tables->magazine_item,"where auto_id='{$_REQUEST["auto_id"]}'");	
		//$backUrl=base64_decode($_REQUEST["backUrl"]);	
		//$this->redirect($backUrl);
