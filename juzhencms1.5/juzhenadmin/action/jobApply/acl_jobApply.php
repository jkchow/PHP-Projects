...
class acl_jobApply extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->job_apply,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
			$recordData["publish"]='1';
			$recordData["job_id"]=$_REQUEST["job_id"];
	...
	function listsData(){
		...
		$where=" from {$dao->tables->job_apply} where 1=1 ";
		
		if($_REQUEST["job_id"]!="")
			$where.=" and job_id='{$_REQUEST["job_id"]}'";
		...
		$list_sql="select auto_id,job_id,content_name,publish,content_mobile,content_email,create_user,create_time "
		.$where.$orderby;
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->job_apply,$dataArray);			
		}else{
			$dao->update($dao->tables->job_apply,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->job_apply,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
	...
	function delete(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			if(is_array($idArr) && count($idArr)>0){
				...
				$idStr=implode(',',$idArr);
				$dao->delete($dao->tables->job,"where auto_id in({$idStr})");
			}
		}
		//$backUrl=base64_decode($_REQUEST["backUrl"]);	
		//$this->redirect($backUrl);
