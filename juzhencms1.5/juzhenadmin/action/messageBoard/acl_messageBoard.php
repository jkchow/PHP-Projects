...
class acl_messageBoard extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->message_board,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
	...
	function listsData(){
		...
		$where=" from {$dao->tables->message_board} where 1=1 ";
		if($_REQUEST["publish"]!='')
		...
		$list_sql="select auto_id,content_title,company_name,content_name,content_desc,publish,create_user,create_time "
		.$where.$orderby;
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->message_board,$dataArray);			
		}else{
			$dao->update($dao->tables->message_board,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function delete(){
		global $dao;
		if($_REQUEST["auto_id"]!="")
			$dao->delete($dao->tables->message_board,"where auto_id='{$_REQUEST["auto_id"]}'");	
		//$this->redirect("?acl={$_REQUEST["acl"]}&method=lists");
