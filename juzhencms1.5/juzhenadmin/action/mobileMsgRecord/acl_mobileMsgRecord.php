...
class acl_mobileMsgRecord extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->mobile_msg_record,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
	...
	function listsData(){
		...
		$where=" from {$dao->tables->mobile_msg_record} where 1=1 ";
		if($_REQUEST["publish"]!='')
		...
		$list_sql="select auto_id,content_mobile,content_result,content_type,create_time "
		.$where.$orderby;
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->mobile_msg_record,$dataArray);			
		}else{
			$dao->update($dao->tables->mobile_msg_record,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function delete(){
		global $dao;
		if($_REQUEST["auto_id"]!="")
			$dao->delete($dao->tables->mobile_msg_record,"where auto_id='{$_REQUEST["auto_id"]}'");	
		//$this->redirect("?acl={$_REQUEST["acl"]}&method=lists");	
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->mobile_msg_record,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
}
