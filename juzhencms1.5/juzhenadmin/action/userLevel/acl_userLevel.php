...
class acl_userLevel extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->user_level,"where auto_id='{$_REQUEST["auto_id"]}'");
			if($recordData["department_id"]=="" || $recordData["department_id"]=="0")
				unset($recordData["department_id"]);
	...
	function treeGridJsonData(){
		...
		$where=" from {$dao->tables->user_level} where auto_code like '____' ";
	...
	private function getSubGridLists($code){
		global $dao;
		$where=" from {$dao->tables->user_level} where auto_code like '{$code}____' ";
	...
	private function getTreeData($where="",$pCode=""){
		...
		global $dao;
		$list_sql="select auto_id,auto_code,name as text from {$dao->tables->user_level} where auto_code like '{$pCode}____' {$whereStr} order by auto_position asc,auto_code asc,auto_id asc ";
		$list=$dao->get_datalist($list_sql);
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$tmp=$dao->get_datalist("select max(right(auto_code,4)) as max_code from {$dao->tables->user_level} where auto_code like '{$_POST["p_code"]}____'");
			...
			$dao->insert($dao->tables->user_level,$dataArray);
			
		}else{
			
			
			
			$dao->update($dao->tables->user_level,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
			...
			if(substr($_POST["auto_code"],0,strlen($dataArray["auto_code"])-4)!=$_POST["p_code"]){
				//如果移动了位置，将排序位设置排在最后
				$dao->update($dao->tables->user_level,array("auto_position"=>time()),"where auto_id='{$_POST["auto_id"]}'");
				...
				$update_sql="update {$dao->tables->user_level} "
	...
	function delete(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			if(is_array($idArr) && count($idArr)>0){
				...
				$idStr=implode(',',$idArr);
				$dao->delete($dao->tables->user_level,"where auto_id in({$idStr})");
			}
		}
		//$this->redirect("?acl={$_REQUEST["acl"]}&method=lists");
	...
	function position_up(){
		if($_REQUEST["auto_id"]!=""){
			...
			$this_record=$dao->get_row_by_where($dao->tables->user_level,"where auto_id='{$_REQUEST["auto_id"]}'",array("auto_id","auto_code","auto_position"));
			$p_code=substr($this_record["auto_code"],0,strlen($this_record["auto_code"])-4);
			//查找上一栏目
			$previous_record=$dao->get_row_by_where($dao->tables->user_level,"where auto_code like '{$p_code}____' and auto_position <='{$this_record["auto_position"]}' and auto_id!='{$_REQUEST["auto_id"]}' order by auto_position desc",array("auto_id","auto_code","auto_position"));
			...
			if(is_array($previous_record)){
				...
				if($this_record["auto_position"]!=$previous_record["auto_position"]){
					...
					$dao->update($dao->tables->user_level,$this_position,"where auto_id='{$this_record["auto_id"]}'");
					$previous_position["auto_position"]=$this_record["auto_position"];
					$dao->update($dao->tables->user_level,$previous_position,"where auto_id='{$previous_record["auto_id"]}'");
	...
	function position_down(){
		if($_REQUEST["auto_id"]!=""){
			...
			$p_code=substr($this_record["auto_code"],0,strlen($this_record["auto_code"])-4);
			//查找下一栏目
			$next_record=$dao->get_row_by_where($dao->tables->user_level,"where auto_code like '{$p_code}____' and auto_position >='{$this_record["auto_position"]}' and auto_id!='{$_REQUEST["auto_id"]}' order by auto_position asc",array("auto_id","auto_code","auto_position"));
			...
			if(is_array($next_record)){
				...
				if($this_record["auto_position"]!=$next_record["auto_position"]){
					...
					$dao->update($dao->tables->user_level,$next_position,"where auto_id='{$next_record["auto_id"]}'");
