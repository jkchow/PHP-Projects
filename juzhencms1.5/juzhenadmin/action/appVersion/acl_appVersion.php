...
class acl_appVersion extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->app_version,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
	...
	function listsData(){
		...
		$where=" from {$dao->tables->app_version} where 1=1 ";
		if($_REQUEST["publish"]!='')
			$where.=" and publish = '{$_REQUEST["publish"]}' ";
		if($_REQUEST["os_type"]!='')
			$where.=" and os_type = '{$_REQUEST["os_type"]}' ";
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->app_version,$dataArray);			
		}else{
			$dao->update($dao->tables->app_version,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function delete(){
		global $dao;
		if($_REQUEST["auto_id"]!="")
			$dao->delete($dao->tables->app_version,"where auto_id='{$_REQUEST["auto_id"]}'");	
		//$this->redirect("?acl={$_REQUEST["acl"]}&method=lists");
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->app_version,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
	
	//app下载的二维码
	function appDownloadQRcode(){
		global $global;
		
		$url=$global->siteUrlPath."/index.php?acl=download&method=downloadApp";
