...
class acl_adSpace extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->ad_space,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
	...
	function listsData(){
		...
		$where=" from {$dao->tables->ad_space} where 1=1 ";
		
		if($_REQUEST["ad_location"]!='')
			$where.=" and ad_location = '{$_REQUEST["ad_location"]}' ";
		...
		$list_sql="select auto_id,content_name,content_width,content_height,content_type,ad_location,position,create_user,create_time "
		.$where.$orderby;
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->ad_space,$dataArray);			
		}else{
			$dao->update($dao->tables->ad_space,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
			
		//$this->redirect("?acl=adSpace&method=lists");	
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->ad_space,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
	...
	function delete(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			if(is_array($idArr) && count($idArr)>0){
				...
				$idStr=implode(',',$idArr);
				$dao->delete($dao->tables->ad_space,"where auto_id in({$idStr})");
			}
		}	
