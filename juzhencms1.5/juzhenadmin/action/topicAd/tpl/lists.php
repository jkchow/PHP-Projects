...
<head>
    ...
    <script type="text/javascript">
		...
		var space_id="<?=$_REQUEST["space_id"]?>";
        var grid = null;
		var dataUrl="?acl="+acl+"&method=listsData&space_id="+space_id;
        ...
        $(function () {			
            grid = $("#maingrid").ligerGrid({
                ...
                { display: '标题', name: 'content_name', minWidth: 100,align:'left' }, 
                //{ display: '类型', name: 'content_type', minWidth: 100,type:"enumVars",enumVars:enumVars.adTypeVars },
				{ display: '链接', name: 'content_link' , minWidth: 60},
		...
		function add_record(){
			var url="?acl="+acl+"&method=form&space_id="+space_id;
			CmsTools.pageJump(url,true);
		}
		function edit_record(recordID){
			if(recordID!=null){
				var url="?acl="+acl+"&method=form&auto_id="+recordID+"&space_id="+space_id;
				CmsTools.pageJump(url,true);
			}else{
				...
				if (!rowdata)
					...
					var url="?acl="+acl+"&method=form&auto_id="+rowdata.auto_id+"&space_id="+space_id;
					CmsTools.pageJump(url,true);
				}
			}
		}
