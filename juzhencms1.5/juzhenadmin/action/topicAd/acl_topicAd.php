...
class acl_topicAd extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->topic_ad,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
			$recordData["publish"]='1';
			$recordData["space_id"]=$_REQUEST["space_id"];
	...
	function listsData(){
		...
		$where=" from {$dao->tables->topic_ad} where topic_id='{$_SESSION["topicId"]}' ";
		
		if($_REQUEST["space_id"]!="")
			$where.=" and space_id='{$_REQUEST["space_id"]}'";
		...
		$list_sql="select auto_id,content_name,content_type,content_link,publish,position,create_user,create_time "
		.$where.$orderby;
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->topic_ad,$dataArray);			
		}else{
			$dao->update($dao->tables->topic_ad,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->topic_ad,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
	...
	function delete(){
		global $dao;
		if($_REQUEST["auto_id"]!="")
			$dao->delete($dao->tables->topic_ad,"where auto_id='{$_REQUEST["auto_id"]}'");	
		//$backUrl=base64_decode($_REQUEST["backUrl"]);	
		//$this->redirect($backUrl);	
