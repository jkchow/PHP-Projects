    ...
    <script type="text/javascript">
        ...
        $(function () {			
            grid = $("#maingrid").ligerGrid({
                ...
                { display: '设备号', name: 'device_id', minWidth: 100,align:'left' }, 
                { display: '设备类型', name: 'app_type', width: 80,type:"enumVars",enumVars:enumVars.appDeviceTypeVars },
				{ display: '登录会员', name: 'member_id', width: 110 },
				{ display: '地区', name: 'city', width: 100 },
                { display: '消息ID', name: 'push_id', width: 110 },
			...
			//设定字典变量的筛选条件
			CmsTools.setSelectOptions({selectId:"app_typeSelect",vars:enumVars.appDeviceTypeVars,emptyText:"不限"});
        });
        function f_search()
        {
...
<form id="searchForm" onsubmit="f_search();return false">
    ...
    &nbsp;
	设备类型：
	<select id="app_typeSelect" name="app_type" style="width:100px;">
    	<option vapp_typealue="">不限</option>
    ...
    <select name="searchFieldName" style="width:100px;">
    	<option value="content_mobile">会员手机号</option>
    	<option value="device_id">设备号</option>
        <option value="push_id">消息ID</option>
