...
class acl_examQuestion extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->exam_question,"where auto_id='{$_REQUEST["auto_id"]}'");			
		}else{
			$recordData=array();
	...
	function listsData(){
		...
		$where=" from {$dao->tables->exam_question} where 1=1 ";
		if($_REQUEST["pid"]!='')
		...
		$list_sql="select auto_id,content_name,publish,position,create_time "
		.$where.$orderby;
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->exam_question,$dataArray);			
		}else{
			$dao->update($dao->tables->exam_question,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function delete(){
		global $dao;
		if($_REQUEST["auto_id"]!="")
			$dao->delete($dao->tables->exam_question,"where auto_id='{$_REQUEST["auto_id"]}'");	
		//$this->redirect("?acl={$this->acl_name}&method=lists");	
		//$backUrl=base64_decode($_REQUEST["backUrl"]);	
		//$this->redirect($backUrl);
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->exam_question,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
