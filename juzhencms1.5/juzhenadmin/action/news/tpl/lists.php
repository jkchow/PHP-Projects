    ...
    <script type="text/javascript">
        ...
        $(function () {			
            grid = $("#maingrid").ligerGrid({
				...
				{ display: '点击量', name: 'content_hit', width: 90 },
				...
				{ display: '发布状态', name: 'publish', width: 90,render:function(rowdata, rowindex, value){
					var vars=enumVars.publishVars;
					if(vars){
						var varItems=vars;
				...
				{ display: '推荐', name: 'recommend', width: 80,render:function(rowdata, rowindex, value){
					...
					if(vars){
						...
						for(var i=0;i<varItems.length;i++){
							var varItem=varItems[i];
							if(varItem.id==value)
								return "<a href='javascript:changeRecommend(" + rowdata.auto_id + ","+value+")'>"+varItem.text+"</a> ";
						}
					}else{
						return "";
					}
		...
		function changeRecommend(recordID,value){
			$.ajax({
			   type: "get",
			   url: "?acl="+acl+"&method=changeRecommend&auto_id="+recordID+"&value="+value,
			   success:function(){
					grid.loadData();
			   }
			});
		...
		function preview_record(){
			...
			if(rows!=null){
				if(rows.length==0){
					...
					var rowdata = grid.getSelectedRow();
					window.open("../index.php?menu="+rowdata.menu_id+"&id="+rowdata.auto_id);
				}		
			}else{
				alert("请先选择要预览的行");
				return;
