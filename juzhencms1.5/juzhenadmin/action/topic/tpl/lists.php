    ...
    <script type="text/javascript">
        ...
        $(function () {			
            grid = $("#maingrid").ligerGrid({
                ...
                { display: '专题名称', name: 'content_name', minWidth: 100,align:'left' }, 
                { display: '地区', name: 'province_id', minWidth: 100 },
                ...
                {
                    var h = "";
					h += "<a href='javascript:enter_topic(" + rowdata.auto_id + ")'>管理</a> ";
		...
		function enter_topic(recordID){
			if(window.parent){
				window.parent.window.location.href="?acl=topic&method=main&id="+recordID;
			}else{
				location.href="?acl=topic&method=main&id="+recordID;
			}
		}
...
<form id="searchForm" onsubmit="f_search();return false">
    ...
    &nbsp;
	地区：
	<select name="province_id" style="width:100px;">
    	<option value="">不限</option>
    	<?
		//案例分类
		$provinceList=$dao->get_datalist("select * from {$dao->tables->address_province} order by id asc");
		if(is_array($provinceList))
		foreach($provinceList as $key=>$vl){
			?>
