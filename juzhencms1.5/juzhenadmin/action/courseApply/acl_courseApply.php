...
class acl_courseApply extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->course_apply,"where auto_id='{$_REQUEST["auto_id"]}'");	
		}else{
			$recordData=array();
	...
	function listsData(){
		...
		$where=" from {$dao->tables->course_apply} where 1=1 ";
		if($_REQUEST["content_status"]!='')
			$where.=" and content_status = '{$_REQUEST["content_status"]}' ";
		if($_REQUEST["content_age"]!='')
			$where.=" and content_age = '{$_REQUEST["content_age"]}' ";
		...
		$childAgeVars=$DictionaryVars->getVars("childAgeVars");
		...
		foreach($list as $key=>$vl){
			...
			//$school=$dao->get_row_by_where($dao->tables->topic,"where auto_id='{$vl["school_id"]}'",array("content_name"));
			//$list[$key]["school_id"]=$school["content_name"];
			
			$list[$key]["content_age"]=$DictionaryVars->getText($childAgeVars,$vl["content_age"]);
		}
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->course_apply,$dataArray);			
		}else{
			$dao->update($dao->tables->course_apply,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function delete(){
		global $dao;
		if($_REQUEST["auto_id"]!="")
			$dao->delete($dao->tables->course_apply,"where auto_id='{$_REQUEST["auto_id"]}'");	
		//$this->redirect("?acl={$_REQUEST["acl"]}&method=lists");	
