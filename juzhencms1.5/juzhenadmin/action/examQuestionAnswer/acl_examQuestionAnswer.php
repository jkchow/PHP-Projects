...
class acl_examQuestionAnswer extends acl_base{
	...
	function form(){
		...
		if($_REQUEST["auto_id"]!=""){
			$recordData=$dao->get_row_by_where($dao->tables->exam_question_answer,"where auto_id='{$_REQUEST["auto_id"]}'");	
	...
	function listsData(){
		...
		$where=" from {$dao->tables->exam_question_answer} where 1=1 ";
		...
		if($_REQUEST["sortname"]!="" && $_REQUEST["sortorder"]!="" ){
			...
			$orderby.=" order by position asc,create_time asc";
		}
		$pagesize=20;
		...
		$list_sql="select auto_id,pid,content_name,publish,position,create_time,create_user "
		.$where.$orderby;
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->exam_question_answer,$dataArray);			
		}else{
			$dao->update($dao->tables->exam_question_answer,$dataArray,"where auto_id='{$_POST["auto_id"]}'");
		}
	...
	function savePosition(){
		...
		if($_REQUEST["auto_id"]!=""){
			...
			$dao->update($dao->tables->exam_question_answer,$dataArray,"where auto_id='{$_REQUEST["auto_id"]}'");		
		}
	}
	...
	function delete(){
		global $dao;
		if($_REQUEST["auto_id"]!="")
			$dao->delete($dao->tables->exam_question_answer,"where auto_id='{$_REQUEST["auto_id"]}'");	
		//$backUrl=base64_decode($_REQUEST["backUrl"]);	
		//$this->redirect($backUrl);
