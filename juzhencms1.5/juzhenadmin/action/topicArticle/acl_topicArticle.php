...
class acl_topicArticle extends acl_base{
	...
	function form(){
		...
		$recordData=$dao->get_row_by_where($dao->tables->topic_article,"where menu_id='{$_REQUEST["menu"]}'");	
	...
	function save(){
		...
		if($_POST["auto_id"]==""){
			...
			$dao->insert($dao->tables->topic_article,$dataArray);			
		}else{
			$dao->update($dao->tables->topic_article,$dataArray,"where menu_id='{$_REQUEST["menu"]}'");
		}
