 ...
 private function copyImage()
 {
  //$size = $this->trueSize();
    ...
    if($destRatio>=$srcRatio){
		$x=0;
		$y=(int)round(($imgHeight-$imgWidth/$destRatio)/2);
		$cutWidth=$imgWidth;
		$cutHeight=(int)round($imgWidth/$destRatio);
	}else{
		$x=(int)round(($imgWidth-$imgHeight*$destRatio)/2);
		$y=0;
		$cutWidth=(int)round($imgHeight*$destRatio);
		$cutHeight=$imgHeight;
	}
  
    imagecopyresized($this->im, $this->dm , 0 , 0 , $x , $y , $this->canvasWidth , $this->canvasHeight , $cutWidth , $cutHeight);
 }
