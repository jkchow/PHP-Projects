...
cache::setCacheMode('1'); //设置缓存模式
if(!$row = cache::get('zj2'))
{  
    $row = array(1,2,3,34,5,6,6);
    cache::set('zj2',$row);
}
//cache::flush(); 清空所有缓存
...
{   
    ...
    public static function setData($id, $data)
    {
        global $global;
		if($global->use_cache){
			return self::set($id, $data,$global->cache_time);
		}else{
			return false;
		}
    }
...
global $global;
cache::setCacheDir($global->fileAbsolutePath."/tmp/cache");
