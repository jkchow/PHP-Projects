<?
class magazineItemNewsService{
	
	
	function getData($id){
		global $dao,$global;
		$data=$dao->get_row_by_where($dao->tables->magazine_item_news,"where auto_id='{$id}'");
	...
	function listsPage($menuId,$magazineItemId,$pagesize=16){
		global $dao,$global;
		$page=$_GET["page"];
		if($page=="")
			$page=1;
		...
		$where=" from {$dao->tables->magazine_item_news} where magazine_item_id='{$magazineItemId}' ";
		
		$orderby=" order by position asc,create_time desc,auto_id desc limit {$begin_count},{$pagesize}";
		
		$list_sql="select auto_id,magazine_item_id,content_name,content_file,create_time "
		.$where.$orderby;
		...
		foreach($list as $key=>$vl){
			if($vl["content_link"]=="")
				$list[$key]["content_link"]=$global->siteUrlPath."/index.php?menu={$menuId}&method=itemNewsDetail&id={$vl["auto_id"]}";
			if($vl["content_file"]!=""){
